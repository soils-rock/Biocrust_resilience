...
ASV_reads_all_bacteria <- read_csv("reads_all_bacteria.csv") |>
  ...
  left_join(taxonomy_all_bacteria %>% select(OTUID, taxon), by = "OTUID") |>
  as.data.frame() |>
  select(!matches("EICP")) |>
  select(!matches("YR2")) |>
...
cyano_w_reads <- qPCR_w_reads |>
  ...
  select(Measurement, Layer, all_of(sample_index))
...
Chl_recov_summary <- Chl_recov_all %>%
  select(Sample_id, type, μgChl_m2, index) %>%
...
complete_table_by_Sample_id$Layer <- factor(complete_table_by_Sample_id$Layer, levels = c("Biocrust", "Subcrust", "Recovered", "B:S Ratio", "Recovery"))
...
complete_table_by_Sample_id <- complete_table_by_Sample_id |>
  mutate(SF = case_when(
    Measurement == "Chlorophyll a" & Layer == "Subcrust" ~ 0.02,
    TRUE ~ SF
  )) |>
  mutate(LC = case_when(
    Measurement == "Chlorophyll a" & Layer == "Subcrust" ~ 0.02,
...
f_complete_table_by_Sample_id <- complete_table_by_Sample_id |>
  ...
  ungroup() |>
  select(-row) # remove helper column
...
for (new_col in names(LV_replicate_groups)) {
  Reads_for_volcano1 <- Reads_for_volcano1 %>%
    mutate(!!new_col := rowMeans(select(., all_of(LV_replicate_groups[[new_col]]))))
}

# Step 3: Remove the original replicate columns (optional)
Reads_for_volcano1 <- Reads_for_volcano1 %>%
  select(-unlist(LV_replicate_groups)) 
...
for (new_col in names(M_replicate_groups)) {
  Reads_for_volcano2 <- Reads_for_volcano2 %>%
    mutate(!!new_col := rowMeans(select(., all_of(M_replicate_groups[[new_col]]))))
}

# Step 3: Remove the original replicate columns (optional)
Reads_for_volcano2 <- Reads_for_volcano2 %>%
  select(-unlist(M_replicate_groups)) 
...
uc_cyano_counts_by_taxa <- UC_cyanos |>
  ...
  select(Taxon, UC_gene_counts) |>
  mutate(across(where(is.numeric), ~ round(.,0)))

#######

#### Calculate # 16S cyano genes in farm soil
vert_no_cyano_reads_table <- read_excel("~/Dropbox (ASU)/ASU/R/Biocrust Recovery/vert_no-cyano_reads_table.xlsx", 
...
vert_uc_counts <- left_join(farm_cyano_counts_by_taxa, uc_cyano_counts_by_taxa, by = "Taxon") |>
  filter(Taxon != "Leptolygbya") |> ## na value
  mutate(farm_pct = 100 * farm_gene_numbers / UC_gene_counts)
...
uc_farm_cyano_genes <- significant_cyanos |>
  ...
  rename("Subcrusts" = "UC_gene_counts") 
...
uc_farm_cyano_genes_with_summary <- uc_farm_cyano_genes |>
  ...
  dplyr::mutate(farm_pct = dplyr::case_when(
    Taxa == "Total" ~ Farm / Subcrusts * 100,
    TRUE ~ farm_pct
  )) |>
  select(1,3,2,4) |>
  rename("Farm/Subcrust (%)" = "farm_pct") |>
  mutate(across(where(is.numeric), ~ round(.,1)))

# Create a flextable
UC_Farm_ft <- flextable(uc_farm_cyano_genes_with_summary) 
UC_Farm_ft <- hline(UC_Farm_ft, i = 11, border = fp_border(width = 2, color = "black"))
UC_Farm_ft <- align(UC_Farm_ft, align = "center", part = "all")
UC_Farm_ft <- color(UC_Farm_ft, color = "black", part = "header") |>
  autofit()

UC_Farm_ft

save_as_docx(UC_Farm_ft, path = "UC_Farm_cyanos.docx")
...
LV_UC_bac_reads <-ASV_reads_all_bacteria |>
  select("OTUID", taxon, LV1UC, LV2UC, LV3UC) |>
  mutate(LVUC = rowMeans(across(c(LV1UC, LV2UC, LV3UC)), na.rm = TRUE)) |>
  select(1,2,6)
...
bundle_16S_model <- lm(cyano_16S ~ bundles_cm2, data = bundle_cyano_16S)
